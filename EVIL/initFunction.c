...
static void initialize_bullet_arry(int id){
	 ...
	 // ovo je deo koda koji se izvrsava kada se ova fja pozove kada unistimo metak...znaci da tim metkom ne mozemo nista da uradimo vise
	 if(id==1){
	 ...
	 else if(id==0){// kada se pozove pritiskom na dugme space svuda stavlja 1 osim na sacuavanom mestu
 ...
 static void initPlayer(){
      ...
      glMaterialfv(GL_FRONT,GL_DIFFUSE,diffuse_coeffs); 
                ...
                glPushMatrix();
                glNormal3f(0.0001,0.0001,0.0001);
            glTranslatef(-0.8,-0.8,0);
            writeName(NULL,-3,-0.9,0);
            glPopMatrix();
      
  
      if(ind_Kill_player==0){
     	  glPushMatrix();
        	glTranslatef(xr,Y_coor_of_Player,0);
                glPushMatrix();
                       glutSolidCube(0.05);
...
static void initGifts(){
	      ...
	      glTranslatef(0,1+gifts1,0);
              glRotatef(3*animation_parametars,0,1,0);
	      glColor3f(1,0,1);
...
static void initEnemyBullets(){
   ...
   if(Ind_Draw_Enemy[5]==1){
    ...
    if(indkiator_za_kraj==1){
    
        
         glPushMatrix();
            glTranslatef(-0.8,0.5,-0.5);
          writeMessage2(NULL,0,0,0) ;
          glPopMatrix();
...
static void initEnemy(){
  ...
  if(36==Win()){
      ...
      if(indikator_za_kraj==0){
    
                glPushMatrix();
                        glTranslatef(0.2+x_coor,0,0);
     ...
     glBindTexture(GL_TEXTURE_2D, 0);
      ...
      else if(indikator_za_kraj==1){
          //ispisujemo koliko je poena sakupio tj paketica
         
                    animation_ongoing=0;
                        glPushMatrix();
            glTranslatef(-0.8,0.5,0);
            writeMessage(NULL,-3,-0.9,0);

            glPopMatrix();

            glPushMatrix();
                glTranslatef(-0.2,0,0);
                writePoints(NULL,-3,-0.7,0);
...
//ispisace nam poruku da smo pobedili ^_^
